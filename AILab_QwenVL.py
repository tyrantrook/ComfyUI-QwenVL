...
def scan_local_models():
    """Scan local LLM directories for Qwen-VL models."""
    local_models = {}
    llm_paths = folder_paths.get_folder_paths("LLM") if "LLM" in folder_paths.folder_names_and_paths else [Path(folder_paths.models_dir) / "LLM"]
    for path in llm_paths:
        qwen_dir = Path(path) / "Qwen-VL"
        if not qwen_dir.exists():
            continue
        for model_dir in qwen_dir.iterdir():
            if model_dir.is_dir():
                # Check for model files
                if any(model_dir.glob("*.safetensors")) or any(model_dir.glob("*.bin")) or any(model_dir.glob("*.pth")):
                    model_name = model_dir.name
                    # Assume local path as repo_id for compatibility
                    local_models[model_name] = {
                        "repo_id": str(model_dir),  # Use local path
                        "default": False,
                        "quantized": False,
                        "vram_requirement": {"full": 8.0, "8bit": 5.0, "4bit": 3.0}  # Default estimates
                    }
                    print(f"[QwenVL] Found local model: {model_name}")
    return local_models
...
def load_model_configs():
    ...
    HF_VL_MODELS.update(local_models)
...
def ensure_model(model_name):
    ...
    if Path(repo_id).is_dir() or (Path(repo_id).exists() and not repo_id.startswith("http")):
        # Local path
        model_path = Path(repo_id)
        if model_path.exists() and (any(model_path.glob("*.safetensors")) or any(model_path.glob("*.bin")) or any(model_path.glob("*.pth"))):
            return str(model_path)
        else:
            raise ValueError(f"Local model path '{repo_id}' does not contain valid model files")
    ...
    if target.exists() and target.is_dir():
        if any(target.glob("*.safetensors")) or any(target.glob("*.bin")) or any(target.glob("*.pth")):
            return str(target)

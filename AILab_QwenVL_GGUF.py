...
def _scan_local_gguf_models(base_dir_path: Path) -> dict[str, dict]:
    """Scan local directories for GGUF model files and their mmproj counterparts."""
    scanned_models = {}
    if not base_dir_path.exists():
        return scanned_models

    # Scan for .gguf files
    for gguf_file in base_dir_path.rglob("*.gguf"):
        model_name = gguf_file.stem  # Remove .gguf extension
        mmproj_candidates = [
            gguf_file.parent / f"mmproj-{model_name}.gguf",
            gguf_file.parent / "mmproj.gguf",
            gguf_file.parent / f"mmproj-{gguf_file.stem}.gguf",
        ]
        mmproj_file = None
        for candidate in mmproj_candidates:
            if candidate.exists():
                mmproj_file = candidate.name
                break

        # Only include if mmproj is found (required for VL models)
        if mmproj_file:
            display_name = gguf_file.name
            scanned_models[display_name] = {
            ...
            print(f"[QwenVL] Found local GGUF model: {display_name} (mmproj: {mmproj_file})")

    return scanned_models

def _load_gguf_vl_catalog():
    if not GGUF_CONFIG_PATH.exists():
        catalog = {"base_dir": "llm/GGUF", "models": {}}
    else:
        try:
            ...
            for repo_key, repo in repos.items():
                ...
                for model_file in model_files:
                    ...
                    if "abliterated" in repo_name.lower():
                        display = f"[ABLITERATED] {display}"
            ...
            catalog = {"base_dir": base_dir, "models": flattened}

    # Scan local models
    base_dir_path = _resolve_base_dir(catalog.get("base_dir") or "llm/GGUF")
    local_models = _scan_local_gguf_models(base_dir_path)
    catalog["models"].update(local_models)
